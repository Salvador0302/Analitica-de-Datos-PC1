...
OUT_CSV = os.path.join(OUT_DIR, "denuncias_2020.csv")
...
def fetch_all_parallel():
    print(f"Iniciando descarga para el filtro: {WHERE}")
    with requests.Session() as session:
        ...
        print(f"IDs divididos en {total_chunks} lotes.")

        print(f"Iniciando descarga con hasta {MAX_WORKERS} hilos...")
        all_gdfs = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for i, future in enumerate(concurrent.futures.as_completed(future_to_chunk_num)):
                ...
                try:
                    ...
                    print(f"Filtro: {WHERE} -> Progreso: {progress:.2f}%")
                except Exception as exc:
                    print(f'El lote {chunk_num + 1} para {WHERE} generó una excepción: {exc}')

    if not all_gdfs:
        return gpd.GeoDataFrame()

    print(f"\nDescarga para {WHERE} completada. Consolidando...")
...
def main():
    ...
    if gdf.empty:
        print(f"No se guardó ningún archivo para {WHERE}.")
        return
    ...
    cols = [c for c in preferred if c in gdf.columns] + [c for c in gdf.columns if c not in (preferred + ["geometry"])]
